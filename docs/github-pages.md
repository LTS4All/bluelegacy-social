...
For a GitHub Pages build, configure the frontend API origin as the hosted project URL, build the client with the normal production command, and publish the generated static output using the repository’s Pages workflow. The account website is intentionally usable from the managed hosted URL first, which is the canonical place to verify registration and the **Completed** confirmation before copying the static build to Pages.
